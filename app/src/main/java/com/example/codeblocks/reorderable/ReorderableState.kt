...
abstract class ReorderableState<T>(
    ...
    internal val interactions = Channel<Pair<() -> Unit, StartDrag>>()
    ...
    private fun calcAutoScrollOffset(time: Long, maxScroll: Float): Float {
        ...
        return when {
            delta > 0 ->
                (endOffset - (viewportEndOffset - 300)).coerceAtLeast(0f)

            delta < 0 ->
                (startOffset - (viewportStartOffset + 300)).coerceAtMost(0f)
