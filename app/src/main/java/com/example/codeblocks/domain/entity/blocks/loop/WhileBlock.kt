    ...
    override fun executeAfterChecks(scope: Scope) {
        ...
        for (block in nestedBlocks) {
            ...
            if (block is BlockWithNesting && block.stopCallingBlock != null) {
                ...
                if (block.stopCallingBlock is ContinueBlock) { return executeAfterChecks(scope) }
                ...
                if (stopCallingBlock is ContinueBlock) { return executeAfterChecks(scope) }
                stopCallingBlock = block
                return
            }
        }

        return executeAfterChecks(scope)
    }
